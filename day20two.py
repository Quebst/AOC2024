...
def main():
    ...
    shorter_paths = find_big_shorts(board)

    counts = Counter(shorter_paths)
    # print(counts)
    count = len(shorter_paths)
    ...
    for i in range(len(board)):
        for j in range(len(board[0])):
            if board[i][j] != "#":
                current = board[i][j]
                # check 20 jumps
                paths.extend(check_short(board, (i, j), current))

    return paths


def check_short(board, cord, current):
    row = cord[0] - 20
    col = cord[1] - 20
    row_edge = len(board)
    ...
    while row <= cord[0] + 20:
        if row > 0 and row < row_edge:
            col = cord[1] - 20
            while col <= cord[1] + 20:
                delta = abs(row - cord[0]) + abs(col - cord[1])
                if col > 0 and col < col_edge and delta <= 20:
                    if board[row][col] != "#":
                        diff = current - delta - board[row][col]
                        if diff >= LIMIT:
                            paths.append(diff)
                col += 1
        row += 1
