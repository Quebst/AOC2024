...
def main():
    ...
    for num in nums:
        answer = num.pop(0)
        if operate(num, answer, "add", 0, 0) or operate(num, answer, "mult", 0, 0) or operate(num, answer, "conc", 0, 0):
            total += answer
            print(answer)
...
def operate(numline, answer, op, accum, index):
    ...
    if len(numline) == index + 1:
        if op == "add":
            ...
            accum = concat(accum, numline[index])
            if accum == answer:
                return True
            else:
                return False
        ...
        if op == "add":
            ...
            accum = concat(accum, left)
        index += 1
        return operate(numline, answer, "add", accum, index) or operate(
            numline, answer, "mult", accum, index) or operate(numline, answer, "conc", accum, index)
